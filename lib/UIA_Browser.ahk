/*
    Source: https://github.com/Descolada/UIAutomation
...
class UIA_Browser {
	...
	SetURL(newUrl, navigateToNewUrl := False) { 
		...
		if (navigateToNewUrl&&InStr(this.URLEditElement.Value, newUrl)) {
			Send "{LCtrl up}{LAlt up}{LShift up}{RCtrl up}{RAlt up}{RShift up}{Ctrl up}{Alt up}{Shift up}"
			KeyWait("Alt")
			KeyWait("Ctrl")
			KeyWait("Shift")
			ControlSend "{Enter}", , "ahk_id" this.BrowserId ; Or would it be better to use BlockInput instead of releasing modifier keys?
		}
	}
